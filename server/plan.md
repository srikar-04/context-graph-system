...
The graph panel uses `react-force-graph-2d`. On mount, it calls `GET /api/graph` and stores the result in React state. The frontend adapts the backend `{ nodes, edges }` payload into the library's `{ nodes, links }` shape before rendering. In practice, the best UX for this dataset is not a raw free-force cluster on first paint. The client should seed a deterministic stage-based layout so the major O2C entity families appear in readable left-to-right bands (`SalesOrder` -> delivery -> billing -> journal -> payment), with master data in their own quieter lanes, and with node positions stable enough that links render immediately on first load instead of only after manual refresh.
